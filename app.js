...
function today(){
  ...
  return layout(`<div class="grid hero-grid">
    ...
    <div class="grid"><section class="card card-pad"><div class="section-title"><h2>${esc(t('emerging.title'))}</h2><span class="sub">${esc(t('emerging.subtitle'))}</span></div><div class="emerging-list">${(r.emerging_signals||[]).map(x=>`<div class="emerging-row"><div class="signal-icon">${safeIcon(x.icon)}</div><div class="emerging-name"><b>${esc(x.name)}</b><small>${esc(x.label||x.reason||'')}</small></div>${sparkline(x.series||[0,0])}<div class="trend">▲ ${x.change||0}%</div></div>`).join('')}</div></section>
...
function impactChain(chain){return `<div class="impact-chain">${chain.map((x,i)=>`${i?'<span class="arrow">→</span>':''}<div class="impact-node"><div class="bubble">${safeIcon(x.icon)}</div>${esc(x.label)}</div>`).join('')}</div>`}
function topicCard(topic){return `<article class="topic-card" style="--accent:${topic.color}"><div class="topic-head"><div class="topic-icon">${safeIcon(topic.icon)}</div><div><b>${esc(topicName(topic))}</b></div></div><div class="topic-count">${esc(t('topics.count',{count:topic.count}))}</div></article>`}
...
function emerging(){const r=state.report;return layout(`<div class="page-title"><div><h1>${esc(t('emerging.title'))}</h1><p>${esc(t('emerging.description'))}</p></div></div><section class="card card-pad"><div class="emerging-list">${(r.emerging_signals||[]).map(x=>`<div class="emerging-row" style="grid-template-columns:50px 1fr minmax(160px,320px) 80px"><div class="signal-icon">${safeIcon(x.icon)}</div><div class="emerging-name"><b>${esc(x.name)}</b><small>${esc(x.reason||x.label||'')}</small></div>${sparkline(x.series||[0,0])}<div class="trend">▲ ${x.change||0}%</div></div>`).join('')}</div></section>`)}
