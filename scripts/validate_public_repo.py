...
def validate_data() -> None:
    ...
    for path in sorted(PUBLIC_DATA.rglob("*.json")):
        ...
        elif path.name == "structures.json" and path.parent.name in {"en", "vi-VN"}:
            # Structural localization files are not report overlays and therefore
            # do not carry demo=true. Their text and references are checked by
            # validate_i18n.py.
            if not isinstance(payload, dict):
                fail(f"localized structures must be an object: {path}")
        else:
