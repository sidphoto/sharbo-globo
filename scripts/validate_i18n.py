...
def validate_structures(path: Path, locale: str) -> None:
    """Validate a structural translation file kept separate from overlays."""
    structure = load_json(path)
    allowed = {"emerging_signals", "impact_chains", "impact_chain"}
    unknown = sorted(set(structure) - allowed)
    if unknown:
        raise AssertionError(f"Unknown structural fields in {path}: {unknown}")
    ...
    def walk(value, location: str) -> None:
        if isinstance(value, dict):
            ...
            raise AssertionError(f"Empty/non-string structural leaf in {path}: {location}")

    walk(structure, locale)
    ...
    for locale in SUPPORTED:
        ...
        for path in sorted(directory.glob("*.json")):
            if path.name == "structures.json":
                validate_structures(path, locale)
            else:
